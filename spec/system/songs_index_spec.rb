RSpec.describe "As a guest" do
  ...
  scenario "I can view songs" do
    ...
    expect(page).to have_text hotel_california.title
    expect(page).to have_text hotel_california.alternate_title
    expect(page).to have_text hotel_california.composer.name
    expect(page).to have_text "On a dark desert highway, cool wind in my hair"
    expect(page).to have_text hotel_california.translation
    expect(page).to have_text hotel_california.categories.map(&:name).to_sentence
  ...
  scenario "I can search across title, lyrics, and composer with consolidated search" do
    ...
    expect(page).to have_text I18n.t("songs.no_songs")

    fill_in I18n.t("songs.search_placeholder"), with: "California"
    click_on "Search Songs"
    expect(page).to have_text hotel_california.title
    expect(page).to have_no_text taki_taki.title
    expect(page).to have_no_text el_condor_pasa.title
  ...
  scenario "I can search for songs by theme" do
    ...
    expect(page).to have_no_text hotel_california.title
  end
  ...
  scenario "per-page limits the number of songs displayed" do
    ...
    expect(page).to have_text("Showing 2 of 3 results")
  end
